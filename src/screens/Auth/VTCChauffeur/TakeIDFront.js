...
const TakeIDFront = () => {
  ...
  const onPermissions = async () => {
    try {
      ...
      // Request only if not granted yet
  ...
  const handleContinue = async () => {
    navigation.navigate("TakeIDBack", {
      frontSide: media,
      type: params?.type,
      id: params?.id,
    });
  ...
  return (
    <ScreenWrapper
      scrollEnabled
      headerUnScrollable={() => <Header title={"Take ID Front"} />}
      footerUnScrollable={() => (
    ...
    >
      ...
      <CustomText
        ...
        textTransform={"none"}
        fontFamily={fonts.medium}
        label={"Position the front of your document facing the camera"}
      />
    </ScreenWrapper>
  );
};

export default TakeIDFront;
